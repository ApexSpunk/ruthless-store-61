...
function Product() {
    ...
    const getProduct = async () => {
        setLoading(true)
        try {
            const response = await fetch(`https://apirest-kkir.onrender.com/products/${id}`);
            const data = await response.json();
            setProduct({ ...data, rating: ((4 + Math.random()).toFixed(1)), reviews: (((60 * Math.random()) * (1 + Math.random())).toFixed(1)) })
            const similar = await fetch(`https://apirest-kkir.onrender.com/products?category=${data.category}&_limit=12`);
